...
/*
 功能：根据路径规划的路径，跟踪无人机的位置相对于下一个目标点的距离，到达阈值后发布下一个点，用于控制无人机的运动。
  输入：路径规划的路径，当前位置，地图到机器人坐标系的转换关系
  输出：目标点，用于控制无人机的运动
  算法：
  1. 订阅路径规划的路径，当前位置，地图到机器人坐标系的转换关系
  2. 循环：
   2.1 等待路径规划的路径，当前位置，地图到机器人坐标系的转换关系
   2.2 计算当前位置到路径上的最近点
   2.3 计算当前位置到最近点的距离
   2.4 如果距离小于目标距离阈值，则发布目标点
   2.5 否则，更新路径规划的路径，当前位置，地图到机器人坐标系的转换关系
  注意：
  路径规划的路径，当前位置，地图到机器人坐标系的转换关系，必须是实时更新的，否则会导致跟踪失败。
*/
